...
# 轻量全局条件生成器
class LayerwiseConditioner(nn.Module):
    def __init__(self, clap_dim=512, bsrnn_channels=128):
        super(LayerwiseConditioner, self).__init__()
        # 将多个条件拼接后，通过 MLP 映射到特征维度
        # 将文本和音频特征映射为统一的条件向量 c_extra
        self.mlp_cond = nn.Sequential(
            nn.Linear(clap_dim * 2, bsrnn_channels),
            nn.SiLU(),
    ...
    def forward(self, f_a, f_t):

        f_concat = torch.cat([f_a, f_t], dim=1) # [B, clap_dim * 2]
        
        # 映射到 BSRNN 的通道维度
        c_extra = self.mlp_cond(f_concat)       # [B, bsrnn_channels]
        
        # 变形为 [B, N, 1, 1] 便于广播
        return c_extra.view(c_extra.size(0), c_extra.size(1), 1, 1)


class BSRNN(nn.Module):
    def __init__(self, num_channel=128, num_layer=6, clap_dim=512):
        super(BSRNN, self).__init__()
        self.num_layer = num_layer
        self.band_split = BandSplit(channels=num_channel)

        self.conditioner = LayerwiseConditioner(clap_dim=clap_dim, bsrnn_channels=num_channel)
        ...
        for m in self.modules():
            if isinstance(m, LayerwiseConditioner):
                continue 
    ...
    def forward(self, x, f_a=None, f_t=None):
        ...
        c_extra = None
        if f_a is not None and f_t is not None:
            c_extra = self.conditioner(f_a, f_t)
