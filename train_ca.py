...
import dataloader_ca
from module_ca import * 
...
parser.add_argument("--cut_len", type=int, default=int(16000*2), help="cut length, default is 2 seconds")
parser.add_argument("--save_model_dir", type=str, default='/home/iasp_guest1/tzx/code/BSRNN/allcf0319', help="dir of saved model")
parser.add_argument("--loss_weights", type=list, default=[0.5, 0.5, 1], help="weights of RI, magnitude, and GAN")
...
class Trainer:
    def __init__(self, train_ds, test_ds):
        ...
        self.model = BSRNN(num_channel=64, num_layer=5, text_dim=768, beats_dim=768).cuda()
        self.discriminator = Discriminator(ndf=16).cuda()
        
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=args.init_lr)
        self.optimizer_disc = torch.optim.Adam(self.discriminator.parameters(), lr=args.init_lr * 0.5)
        self.window = torch.hann_window(self.n_fft).cuda()

    def get_latest_ckpt(self):
        """自动检索目录下最新的 generator 和 discriminator 权重"""
    ...
    def train_step(self, batch, use_disc):
        ...
        est_spec = self.model(noisy_spec, f_a_seq=f_a_seq, f_t=f_t)
    ...
    def train(self):
        ...
        def lr_lambda(epoch):
            if epoch < args.decay_epoch:
                ...
                return 0.96 ** (epoch - args.decay_epoch)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for idx, batch in enumerate(tqdm(self.train_ds)):
                ...
                if (step % args.log_interval) == 0:
                    ...
                    logging.info(template.format(epoch, step, loss_total/step, loss_gan/step) + f" | LR: {current_lr:.6f}")
...
def main():
    ...
    train_ds, test_ds = newdataloader.load_data(args.batch_size, 4, args.cut_len)
